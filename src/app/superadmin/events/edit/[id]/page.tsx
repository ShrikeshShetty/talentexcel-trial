...
import { updateEvent, getEventById, uploadEventImage, Event, EventType, EventCategory, EventStatus, isSuperAdmin } from '@/lib/superadmin';
...
export default function EditEvent() {
  ...
  const [useExternalUrls, setUseExternalUrls] = useState<boolean>(false);

  // Check authentication and fetch event data on load
  useEffect(() => {
    const checkAuthAndFetchEvent = async () => {
      try {
        ...
        if (event) {
          ...
          const formattedStartDate = event.start_date ? new Date(event.start_date).toISOString().slice(0, 16) : '';
          const formattedEndDate = event.end_date ? new Date(event.end_date).toISOString().slice(0, 16) : '';
          ...
          // If event has existing URLs, default to external URLs mode
          if (event.logo_url || event.image_url) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let finalLogoUrl = eventData.logo_url;
      ...
      if (useExternalUrls) {
        ...
        if (logoFile) {
          try {
            const uploadedLogoUrl = await uploadEventImage(logoFile, 'logo');
            if (uploadedLogoUrl) {
              finalLogoUrl = uploadedLogoUrl;
            }
          } catch (error) {
            console.warn('Logo upload failed, using existing URL:', error);
          }
        }

        if (imageFile) {
          try {
            const uploadedImageUrl = await uploadEventImage(imageFile, 'image');
            if (uploadedImageUrl) {
              finalImageUrl = uploadedImageUrl;
            }
          } catch (error) {
            console.warn('Image upload failed, using existing URL:', error);
          }
        }
      }

      // Update event with URLs
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="sm:col-span-3">
                        ...
                        {eventData.logo_url && !logoFile && (
                          <div className="mt-2 flex items-center">
                            ...
                            <span className="ml-2 text-sm text-gray-500">Current logo (will be kept if no new file is uploaded)</span>
                          </div>
                        )}
                        <p className="mt-1 text-sm text-gray-500">
                      ...
                      <div className="sm:col-span-3">
                        ...
                        {eventData.image_url && !imageFile && (
                          <div className="mt-2 flex items-center">
                            ...
                            <span className="ml-2 text-sm text-gray-500">Current banner (will be kept if no new file is uploaded)</span>
                          </div>
                        )}
                        <p className="mt-1 text-sm text-gray-500">
                ...
                <div className="flex justify-end space-x-3">
                  <Link
                    href={`/superadmin/events/${eventId}`}
